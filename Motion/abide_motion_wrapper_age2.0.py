...
def abide_motion_wrapper(motion_thresh, age_l, age_u, n, n_perms=1000, overwrite=True):
    ...
    f_name = 'RESULTS_age2.0/rsq_{:03.0f}pct_{:03.0f}subs_{:02.0f}to{:02.0f}.csv'.format(motion_thresh, n, age_l, age_u)
    ...
    rsq_list, icc_list, pear_r_list, spear_list = split_half_outcome(df, motion_thresh, age_l, age_u, n, n_perms=n_perms)
    ...
    med_pear_r = np.median(pear_r_list)
    pear_r_CI = np.percentile(pear_r_list, 97.5) - np.percentile(pear_r_list, 2.5)
    
    med_spear_p = np.median(spear_p_list)
    spear_r_CI = np.percentile(spear_r_list, 97.5) - np.percentile(spear_r_list, 2.5)

    columns = [ 'motion_thresh', 'age_l', 'age_u', 'n', 'med_rsq', 'CI_95', 'med_icc', 'CI_95_icc', 'med_pear_r','pear_r_CI','med_spear_r','spear_R_CI' ]
    results_df = pd.DataFrame(np.array([[motion_thresh, age_l, age_u, n, med_rsq, rsq_CI, med_icc, icc_CI, med_pear_r,pear_r_CI, med_spear_r, spear_r_CI]]), 
                                  columns=columns)
...
def split_half_outcome(df, motion_thresh, age_l, age_u, n, n_perms=100):
    ...
    for i in range(n_perms):
        ...
        ##calculate the age to roi-roi correlations across the two samples
        age_roi_corr_r_A, age_roi_corr_p_A, age_roi_corr_m_A = corr_w_age(df_A, all_corr_mat_A)
        age_roi_corr_r_B, age_roi_corr_p_B, age_roi_corr_m_B = corr_w_age(df_B, all_corr_mat_B)
        
        ##calculate the R value between split half roi-roi~age correlation matrices. Ignore the Nans
        indices = np.triu_indices_from(all_corr_mat_A, k=1)
        ...
        # calc pearson r
        pear_r, p = pearsonr(grA_values[~mask_nans], grB_values[~mask_nans])
        
        #calc spearman r
        spear_r, p = spearmanr(grA_values[~mask_nans], grB_values[~mask_nans])
        ...
        #build the r and p output for age correlations
        pear_r_list += [pear_r]
        spear_r_list += [spear_r]
    
    return np.array(Rsq_list), np.array(ICC_list),np.array(pear_r_list), np.array(spear_p_list)
...
def corr_w_age(df, all_corr_mat):
    ...
    for i, j in zip(i_list, j_list):
        age_roi_corr_r[i, j] = pearsonr(all_corr_mat[i, j, :], age)[0]
        age_roi_corr_p[i, j] = pearsonr(all_corr_mat[i, j, :], age)[1]
        age_roi_corr_m[i, j] = linregress(all_corr_mat[i, j, :], age)[0]
